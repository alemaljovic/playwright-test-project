...
test("Registration", async ({ page }) => {
  ...
  await page.locator('input[placeholder="Ime"]').first().fill("Alem");
  await page.locator('input[placeholder="Prezime"]').fill("Aljovic");
  await page
    .getByRole("textbox", { name: "Email:" })
    .fill("alemaljovi58@gmail.com");
...
async function login(page: Page) {
  ...
  await page
    .getByRole("textbox", { name: "Email:" })
    .fill("alemaljovi58@gmail.com");
  await page.getByRole("textbox", { name: "Lozinka:" }).fill("Alem123");
...
test("Newsletter", async ({ page }) => {
  ...
  await page.getByPlaceholder("Unesite email").fill("alemalem@gmail.com");
  await page.getByRole("button", { name: "Newsletter prijava" }).click();
});
...
test("Gift Card", async ({ page }) => {
  ...
  await page
    .getByRole("link", { name: "BONUS UZ BUZZ GIFT CARD –" })
    .first()
    .click();
});
...
test("Account-settings", async ({ page }) => {
  await login(page);
  await page.getByRole("link", { name: "Alem Aljovic" }).click();
